...
int peripherals_init(void)
{
	...
	if (IS_ENABLED(CONFIG_BOARD_NRF5340_AUDIO_DK_NRF5340_CPUAPP)) {
	...
	} else if (IS_ENABLED(CONFIG_BOARD_MUSIC_PLAYER_NRF5340_CPUAPP)) {
		/* Simpler SD initialization for custom music player board */
		ret = sd_card_init();
		if (ret != -ENODEV && ret != 0) {
			LOG_ERR("Failed to initialize SD card");
			return ret;
		...
		ret = music_player_hw_init();
		if (ret) {
			LOG_ERR("Failed to initialize music player hardware");
			return ret;
		}
	}
